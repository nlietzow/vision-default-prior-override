...
class PatchingLastTokenResStream(VMIRunnerBase):
    runner_name = "patching_last_token_res_stream"
    ...
    def _forward_and_cache(
        ...
        with self.nnsight.trace() as tracer:  # noqa: SIM117
            with tracer.invoke(**inputs):
                for layer in self.get_language_layers():
                    act = self.adapter.get_layer_output_last_token(layer).save()
                    activations.append(act)
                outputs = self.adapter.get_lm_head_output_saved(self.nnsight)

        return outputs.cpu().clone(), [act.cpu().clone() for act in activations]
    ...
    def _forward_with_patching(
        ...
        with self.nnsight.trace() as tracer:  # noqa: SIM117
            with tracer.invoke(**inputs):
                ...
                self.adapter.set_layer_output_last_token(layer, activations_to_patch)
                outputs = self.adapter.get_lm_head_output_saved(self.nnsight)
