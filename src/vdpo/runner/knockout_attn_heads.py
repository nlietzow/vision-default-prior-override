...
class KnockoutAttnHeads(VMIRunnerBase):
    runner_name = "knockout_attn_heads"
    ...
    def __init__(
        ...
        *,
        ...
        self._zeros = torch.zeros(self.head_dim, device=self.model.device)
        ...
        for t in self.promoting + self.suppressing:
            kid = f"ind_L{t['layer_idx']}H{t['head_idx']}"
        ...
        with self.nnsight.trace() as tracer:  # noqa: SIM117
            with tracer.invoke(**inputs):
                ...
                for target in sorted_targets:
                    ...
                    start = target["head_idx"] * self.head_dim
                    end = start + self.head_dim
                    self.adapter.set_o_proj_input_slice(layer, start, end, self._zeros)
                outputs = self.adapter.get_lm_head_output_saved(self.nnsight)
        return outputs.cpu().clone()
