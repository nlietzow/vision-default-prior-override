"""Appendix figure: Residual stream restoration scores for all 5 models under the visual-circuit contrast.

Five-panel line plot showing S2T (dashed) and T2S (solid) restoration curves
with critical window shading. Mirrors fig_app_res_stream_all.py but points at
the visual_circuit subdirectory of the residual-stream patching outputs.

Output: figures/res_stream_all_models_visual_circuit.pdf
"""
...
OUTPUT_DIR = settings.output_dir / "patching_last_token_res_stream" / "visual_circuit"


def main():
    setup_style()

    print("Loading visual-circuit results...")
    ...
    for ax, model in zip(axes, models, strict=False):
        ...
        a_key = (
            ...
            if "results_source_to_target" in rs[0]
            else "results_p2v"
        )
        n_layers = len(rs[0][a_key])
        s2t_vals, t2s_vals = compute_mean_restoration_curves(rs)

        avg = [(s + t) / 2 for s, t in zip(s2t_vals, t2s_vals, strict=False)]
        ...
        ax.plot(layers, t2s_vals, color=V2P_COLOR, linewidth=1.0, label="CF$\\to$Orig")
        ax.plot(
            layers,
            s2t_vals,
            color=P2V_COLOR,
    ...
    save_fig(fig, "res_stream_all_models_visual_circuit.pdf")
    print("Done.")
